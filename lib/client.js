			...
			function attempt(settingsScope, where) {
				...
				try {
					ctx.effect(() => slots.inject('settings.section', () => {
						...
						// 这条才是确定信号：slots.inject 是**延迟**执行的，只有槽位被声明时
						// 回调才跑。前一条只说明"提交了注册"，这一条说明"注册真的执行了"。
						reportDiagnostic(`settings-section: register 回调已执行（${where}）`)
						return dispose
					}), 'peak-brief: settings section')
				} catch (error) {
					reportFailure(`${where}：slots.inject 失败 —— ${error?.message ?? error}`)
					return false
				}
				reportDiagnostic(`settings-section: 已提交注册（${where}，等待槽位声明）`)
				return true
			}
