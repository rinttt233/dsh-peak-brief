...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** 把设置页挂进 Settings 的 section 槽位。返回是否挂上。 */
		function registerSettingsSection(ctx, slots) {
			// 优先用 ctx.settingsScope（该服务代理在**调用时**把 this 绑到调用者，
			// 写入才会挂在我们的 fiber 上）。但直接访问未声明的服务在 cordis 里会抛
			// "without inject"，所以先试属性、再退回 ctx.get（官方插件 ctx.get('slots')
			// 就是这个套路，客户端不做硬 inject）。
			let settingsScope
			try {
			...
			try {
				...
				console.warn(`[peak-brief] 绑定设置命名空间失败：${error?.message ?? error}`)
				return false
			}
		...
		function apply(ctx) {
			...
			if (hasSlots) {
				...
				registerSettingsSection(ctx, slots)
				return
			}
